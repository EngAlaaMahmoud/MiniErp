namespace MiniErp.Api.IntegrationTests;

public static class TestConstants
{
    public static readonly Guid TenantId = Guid.Parse("11111111-1111-1111-1111-111111111111");
    public static readonly Guid BranchId = Guid.Parse("22222222-2222-2222-2222-222222222222");
    public static readonly Guid DeviceId = Guid.Parse("33333333-3333-3333-3333-333333333333");
    public static readonly Guid OwnerUserId = Guid.Parse("44444444-4444-4444-4444-444444444444");

    public static readonly Guid MilkProductId = Guid.Parse("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa");
}

